...
\section*{Task 22}
The controller worked as it should in the simulations but did not work when implemented in the physical system. More precisely, the rotational controller worked when implemented but when the system switched state to the line following part, it did not work. The simulation results when following the track have been presented in Figure~\ref{fig:task19_cont} and for the sake of completeness, the experimental results are displayed, though they do not contribute to the controller evaluation. The group hypothesize that the controller might be flawed but working during ideal conditions but when subjected to real life disturbances, the low robustness of the controll renders the system unstable and unusable.
